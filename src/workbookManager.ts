...
export const generateTableWorkbook = async (initialDataGrid: Grid, docProps?: DocProps): Promise<Blob> => {
    ...
    await updateWorkbookInitialDataIfNeeded(zip, docProps, tableData);
...
const generateSingleQueryWorkbookFromZip = async (
    ...
    await updatePowerQueryDocument(zip, query.queryName, generateSingleQueryMashup(query.queryName, query.queryMashup));
    await updateSingleQueryAttributes(zip, query.queryName, query.refreshOnOpen);
    await updateWorkbookInitialDataIfNeeded(zip, docProps, tableData, true /*updateQueryTable*/);
...
const updateSingleQueryAttributes = async (zip: JSZip, queryName: string, refreshOnOpen: boolean) => {
    // Update connections
