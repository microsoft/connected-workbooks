...
export default class MashupHandler {
    ...
    private editSingleQueryMetadata = (metadataArray: Uint8Array, metadata: Metadata) => {
        ...
        const entries: HTMLCollectionOf<Element> = parsedMetadata.getElementsByTagName("Entry");
            if (entries && entries.length) {
                for (let i = 0; i < entries.length; i++) {
                    ...
                    const entryProp: Attr | undefined = entryAttributesArr.find((prop) => {return prop?.name === "Type"});
