...
import { documentUtils, tableUtils } from "../src/utils";
...
describe("Table Utils tests", () => {
    test("tests workbookXML contains initial data dimensions", () => {
        ...
        const cellRangeRef = defaults.sheetName + tableUtils.GenerateReferenceFromString(`A1:${documentUtils.getCellReferenceRelative(tableData.columnNames.length - 1, tableData.rows.length + 1)}`)
        const defaultString =
        ...
        const worksheetXml = tableUtils.updateWorkbookInitialData(defaultString, cellRangeRef)
    ...
    test("tests Pivot Tables contain initial data", () => {
        ...
        const tableXmlSheet = tableUtils.updateTablesInitialData(
            defaultString,
            tableData,
            `A1:${documentUtils.getCellReferenceRelative(tableData.columnNames.length - 1, tableData.rows.length + 1)}`,
            true
        );
    ...
    test("test valid initial data in SheetsXML", () => {
        ...
        const sheetsXmlString = tableUtils.updateSheetsInitialData(defaultString, tableData,
            `A1:${documentUtils.getCellReferenceRelative(tableData.columnNames.length - 1, tableData.rows.length + 1)}`);
