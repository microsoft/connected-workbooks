...
describe("Table Utils tests", () => {
    test("tests workbookXML contains initial data dimensions", async () => {
        ...
        const worksheetXml = await tableUtils.updateWorkbookInitialData(defaultString, {
    ...
    test("tests Pivot Tables contain initial data", async () => {
        ...
        const tableXmlSheet = await tableUtils.updateTablesInitialData(
            defaultString,
            {
                columnNames: ["Column1", "Column2"],
                rows: [["1", "2"]],
    ...
    test("test valid initial data in SheetsXML", async () => {
        ...
        const sheetsXmlString = await tableUtils.updateSheetsInitialData(defaultString, {
            columnNames: ["Column1", "Column2"],
            rows: [["1", "2"]],
        });
    ...
    test("tests Query Tables contain initial data", async () => {
        ...
        const queryTableXmlSheet = await tableUtils.updateQueryTablesInitialData(defaultString, {
            columnNames: ["Column1", "Column2"],
            rows: [["1", "2"]],
        });
    ...
    test("tests Get adjusted column Name", async () => {
        ...
        expect(await tableUtils.getAdjustedColumnNames(["Column", "Column", "Column (2)", "Column (3)"])).toEqual([
            "Column",
            "Column (1)",
            "Column (2)",
            "Column (3)",
